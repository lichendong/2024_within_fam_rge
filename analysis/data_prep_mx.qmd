...
psych::alpha(adhd |> select(-AID),check.keys=TRUE) |> suppressWarnings()

# psych::fa.parallel(adhd |> select(-AID), fm = "wls", fa = "fa", cor = "poly")

# lavaan::efa(adhd |> select(-AID) |> mutate(across(everything(), as.ordered)), nfactors = 1:2, rotation = "oblimin") |> summary()
...
```{r}
names_parent_p <- c("PC34A", "PC34B", "PC34C", "PC34D", "PC34E", "PC44A")
fa_parent_p <- mxModel("fa_parent_p",
                       mxData(observed=data |> select(PC34A:PC44A), type='raw'),
                       mxPath(from="f1", arrows=2, free=F, values=1, labels ="var_f1"),
                       mxPath(from="one", to = "f1", arrows=1, free=F, values=0, labels ="m_f1"),
                       mxPath(from="f1", to = names_parent_p, arrows=1, free=c(FALSE,TRUE,TRUE), 
                              values=1, labels= paste("loading", names_parent_p, sep = "_")),
                       )
```
...
fs_all <- fs_1 |> left_join(fs_2) |> left_join(fs_3) |> 
                  ...
                  left_join(fs_6) |> left_join(fs_adhd) |> 
          mutate(AID = as.character(AID))
```
